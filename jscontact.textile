In this advanced Rails project you'll create a contact manager while practicing techniques including:

* Using "RSpec":http://rspec.info/ and tests to drive your development
...
* Building complex forms with "Formtastic":http://github.com/justinfrench/formtastic
...
This project assumes you have already completed the "general Ruby setup":http://jumpstartlab.com/resources/general/environment/ and "general Rails setup":http://jumpstartlab.com/resources/rails-jumpstart/preparation/ procedures.  In addition, we'll use several gems specific to this tutorial.  While the Bundler system can download them for you on the fly, pre-installing them will save some time.  On Linux or MacOS, use the command below.  On Windows, use the same command but omit the "sudo" at the beginning:

<pre class='brush:plain'>
sudo gem install haml nifty_generators rspec-rails cucumber webrat mocha carrierwave simpleform heroku
</pre>
...
Run the @rails -v@ command and you should see your current Rails version, mine is *3.0.6*.  Let's create a new Rails project:
...
With our project created, I'd like to veer off the Rails defaults for both the testing and JavaScript libraries.  First, let's setup RSpec.  Open your project's @Gemfile@ and add this dependency:
...
Now rspec-rails is available, but your project isn't set to use it by default.  Accomplish that change by running this generator:
...
Now your project generators will use RSpec by default.  You still have Rails' default @test@ folder hanging around -- let's delete that with:
...
h3. Then Replacing Prototype with jQuery
...
Run @bundle@ again, then run this generator from your project directory:
...
Overwrite /Users/jcasimir/Dropbox/Projects/JSContact/public/javascripts/rails.js? (enter "h" for help) [Ynaqdh] y
...
Run @bundle@ from your project directory, wait for the installation.  Then to start the server:

<pre class="brush:plain">
  unicorn
</pre>

And you're running!  Load "http://0.0.0.0:8080":http://0.0.0.0:8080 in your browser and you should see the Rails splash screen.  Click the "About Your Application" link and you should see all your library versions.  If your database were not installed properly or inaccessible, you'd see an error here.
...
Then add your current working directory to the repository and commit it:
...
Then you'll get back the URL where the app is accessible.  Try loading it in your browser and you'll see the generic Heroku splash screen.  It's not yet running your code!  Push your project up like this:
...
I want to use the scaffold generator to create a model named @Person@, but I don't care for some of the conventions used in the default scaffold generator.  Instead, I prefer the @nifty-generators@ created by "RailsCasts author Ryan Bates":http://railscasts.com.
...
The first line of the output shows Ryan sneaking the @"mocha"@ gem into our Gemfile.  Move the gem dependency into the development/test block we already have and run @bundle@.  Then run the migration with @rake db:migrate@.

Both of the generators created test-related files for us.  They saw that we're using RSpec and created corresponding controller and model test files.  Let's run those tests now:
...
Try creating a few sample people on @localhost@ through your browser.
...
If you run @rake@ here you'll see that the test is _still_ failing.  Huh?  Try running it like this:
...
Following that concept, let's add a validation for @:last_name@ to the model then write a test checking that a person is not valid without a last name.
...
Next open the @person.rb@ model and add the association @has_many :phone_numbers@.  Go into the @phone_number.rb@ model and add the relationship @belongs_to :person@.  Run @rake@ and your tests should all pass.

Try it out yourself by going into the console and adding a phone number manually:
...
Many developers don't test helpers, but I find they're one of the most common failure points in production applications.  Devs don't write tests for them because they're "just little presentation methods," then you see weird presentation artifacts in the application as the helpers go wrong.
...
Now @rake@ your tests and they should all pass.  Since you're green it be a good time to *commit your changes*.
